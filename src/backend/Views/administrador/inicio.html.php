<div id="requiere-cambio-clave">
  <input type="hidden" id="cambio-clave" value="<?=$usuario->cambio_clave ?? true;?>">
</div>
<div class="d-flex w-100 h-100 justify-content-center mt-4">
  <div class="w-100 h-90 me-3 d-flex justify-content-center align-items-end">
      <div class="cuadro-blaco-p mb-2 d-flex flex-column">
        <div class="flex-grow-1 d-flex justify-content-center align-items-center" id="contenido">
              <div class="w-100 h-100 m-3">
                  <div class="w-100 mt-4">
                    <h3 class="text-center tipografia-sanSerif-4">
                    ADMINISTRADOR DEL SISTEMA SEAC
                    </h3>
                    <p class="text-center tipografia-sanSerif-1">El administrador del sistemas SEAC podra realizar diferentes acciones dentro del mismo.</p>
                  </div>
                  <div class="w-100 d-flex justify-content-around gap-6 tipografia-sanSerif-1 flex-wrap">
                    <div class="d-flex flex-column justify-content-center align-items-center ancho-tarjetas-1">
                      <img class="img-inicio-options-home" src="/public/assets/img/undraw_chore_list_re_2lq8.svg" alt="files">
                      <section>
                        <h5 class="tipografia-sanSerif-4 mt-3 text-center">
                        PERIODO ACADÉMICO:
                        </h5>
                        <p class="text-justificado">
                        Permitirá al administrador registrar, editar y listar el periodo académico en el que se encuentre la Universidad.
                        </p>
                    </section>
                    </div>
                    <div class="d-flex flex-column justify-content-center align-items-center ancho-tarjetas-1">
                      <img class="img-inicio-options-home" src="/public/assets/img/undraw_city_girl_ccpd.svg" alt="test">
                      <section>
                        <h5 class="tipografia-sanSerif-4 mt-3">
                        FACULTADES:
                        </h5>
                        <p class="text-justificado">
                        Permitirá al administrador registrar, editar y listar.
                        </p>
                      </section>
                    </div>

                    <div class="d-flex flex-column justify-content-center align-items-center ancho-tarjetas-1">
                      <img class="img-inicio-options-home" src="/public/assets/img/undraw_education_f8ru.svg" alt="test">
                      <section>
                        <h5 class="tipografia-sanSerif-4 mt-3">
                        CARRERAS:
                        </h5>
                        <p class="text-justificado">
                        Permitirá al administrador registrar, editar y listar.
                        </p>
                      </section>
                    </div>

                    <div class="d-flex flex-column justify-content-center align-items-center ancho-tarjetas-1">
                      <img class="img-inicio-options-home" src="/public/assets/img/undraw_back_in_the_day_knsh.svg" alt="test">
                      <section>
                        <h5 class="tipografia-sanSerif-4 mt-3">
                        COORDINADOR DE CARRERA:
                        </h5>
                        <p class="text-justificado">
                        Permitirá al administrador registrar, editar y listar.
                        </p>
                      </section>
                    </div>
                  </div>
              </div>
              
        </div>
      </div>
  </div>
</div>