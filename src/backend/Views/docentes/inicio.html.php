<div class="d-flex w-100 h-100 justify-content-center mt-4">
  <div class="w-100 h-90 me-3 d-flex justify-content-center align-items-end">
      <div class="cuadro-blaco-p mb-2 d-flex flex-column">
        <div class="flex-grow-1 d-flex justify-content-center align-items-center" id="contenido">
              <div class="w-100 h-100 m-3">
                  <div class="w-100 mt-4">
                    <h3 class="text-center tipografia-sanSerif-4">
                    DOCENTE DE LA CARRERA
                    </h3>
                    <p class="text-center tipografia-sanSerif-1">El docente encargado de almacenar las evidencias de la carrera puede realizar diferentes acciones dentro del sistema.</p>
                  </div>
                  <div class="w-100 d-flex justify-content-around gap-6 tipografia-sanSerif-1 flex-wrap">
                    <div class="d-flex flex-column justify-content-center align-items-center ancho-tarjetas-1">
                      <img class="img-inicio-options-home" src="/public/assets/img/undraw_at_work_re_qotl.svg" alt="files">
                      <section>
                        <h5 class="tipografia-sanSerif-4 mt-3 text-center">
                        EVIDENCIAS:
                        </h5>
                        <p class="text-justificado">
                        El docente puede visualizar la lista de evidencias que estén bajo su responsabilidad y también podrá subirlas para su respectivo almacenamiento.
                        </p>
                    </section>
                    </div>
                    <div class="d-flex flex-column justify-content-center align-items-center ancho-tarjetas-1">
                      <img class="img-inicio-options-home" src="/public/assets/img/undraw_diary_re_4jpc.svg" alt="test">
                      <section>
                        <h5 class="tipografia-sanSerif-4 mt-3">
                        REPORTES:
                        </h5>
                        <p class="text-justificado">
                        El docente puede generar reportes de las evidencias almacenadas y no almacenadas.
                        </p>
                      </section>
                    </div>

                    <div class="d-flex flex-column justify-content-center align-items-center ancho-tarjetas-1">
                      <img class="img-inicio-options-home" src="/public/assets/img/undraw_fresh_notification_re_whq4.svg" alt="test">
                      <section>
                        <h5 class="tipografia-sanSerif-4 mt-3">
                        NOTIFICACIONES:
                        </h5>
                        <p class="text-justificado">
                        En esta sección se puede observar las notificaciones que le han remitido acerca de la documentación almacenada. 
                        </p>
                      </section>
                    </div>
                  </div>
              </div>
              
        </div>
      </div>
  </div>
</div>