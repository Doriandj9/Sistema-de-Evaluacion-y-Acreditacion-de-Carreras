<div id="requiere-cambio-clave">
  <input type="hidden" id="cambio-clave" value="<?=$usuario->cambio_clave ?? true;?>">
</div>
<div class="d-flex w-100 h-100 justify-content-center mt-4">
  <div class="w-100 h-90 me-3 d-flex justify-content-center align-items-end">
      <div class="cuadro-blaco-p mb-2 d-flex flex-column">
        <div class="flex-grow-1 d-flex justify-content-center align-items-center" id="contenido">
              <div class="w-100 h-100 m-3">
                  <div class="w-100 mt-4">
                    <h3 class="text-center tipografia-sanSerif-4">
                    DIRECTOR DE PLANEAMIENTO
                    </h3>
                    <p class="text-center tipografia-sanSerif-1">El director de planeamiento puede realizar las diferentes acciones que se mencionan en el menú.</p>
                  </div>
                  <div class="w-100 d-flex justify-content-around gap-6 tipografia-sanSerif-1 flex-wrap">
                    <div class="d-flex flex-column justify-content-center align-items-center ancho-tarjetas-1">
                      <img class="img-inicio-options-home" src="/public/assets/img/undraw_data_processing_yrrv.svg" alt="files">
                      <section>
                        <h5 class="tipografia-sanSerif-4 mt-3 text-center">
                        BASE DE INDICADORES:
                        </h5>
                        <p class="text-justificado">
                        En esta sección se puede visualizar la base de indicadores almacenada completamente.
                        </p>
                    </section>
                    </div>
                    

                    <div class="d-flex flex-column justify-content-center align-items-center ancho-tarjetas-1">
                      <img class="img-inicio-options-home" src="/public/assets/img/undraw_empty_re_opql.svg" alt="test">
                      <section>
                        <h5 class="tipografia-sanSerif-4 mt-3">
                        EMPAREJAMIENTO DE EVALUADORES:
                        </h5>
                        <p class="text-justificado">
                        Se puede observar la lista de los evaluadores y también se puede realizar el emparejamiento para la evaluación de las carreras.
                        </p>
                      </section>
                    </div>

                    <div class="d-flex flex-column justify-content-center align-items-center ancho-tarjetas-1">
                      <img class="img-inicio-options-home" src="/public/assets/img/undraw_diary_re_4jpc.svg" alt="test">
                      <section>
                        <h5 class="tipografia-sanSerif-4 mt-3">
                        REPORTES:
                        </h5>
                        <p class="text-justificado">
                        Permite generar un reporte sobre la evaluación realizada por los evaluadores a cada una de las carreras.
                        </p>
                      </section>
                    </div>
                  </div>
              </div>
              
        </div>
      </div>
  </div>
</div>